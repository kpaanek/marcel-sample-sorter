...
public class SampleMockRepository implements SampleRepository {
    ...
    private static final Map<Long, Sample> SAMPLE_MAP = new ConcurrentHashMap<>() {
        {
            put(1L, new Sample(2, new PatientDetails(
                    30, "COMPANY_B", "DISTRICT_B", "DEFECT_B"
            )));
            put(2L, new Sample(3, new PatientDetails(
                    35, "COMPANY_A", "DISTRICT_B", "DEFECT_B"
            )));
            put(3L, new Sample(4, new PatientDetails(
                    35, "COMPANY_B", "DISTRICT_A", "DEFECT_B"
            )));
            put(4L, new Sample(5, new PatientDetails(
